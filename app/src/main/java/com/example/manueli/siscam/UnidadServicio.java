...
public class UnidadServicio extends ActionBarActivity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_unidad_servicio);
                ...
                if (Seleccionado != "Seleccionar Mantenimiento.."){
                    try{
                        ...
                        if (con == null) {
                            ...
                            ArrayAdapter NoCoreAdapter2 = new ArrayAdapter(UnidadServicio.this, android.R.layout.simple_expandable_list_item_1, data2);
                        ...
                        inputMethodManager2.hideSoftInputFromWindow(edtCosto.getWindowToken(), 0);
                        InputMethodManager inputMethodManager3 = (InputMethodManager)getSystemService(Context.INPUT_METHOD_SERVICE);
                        inputMethodManager3.hideSoftInputFromWindow(edtObservacionProveedor.getWindowToken(), 0);
                        InputMethodManager inputMethodManager4 = (InputMethodManager)getSystemService(Context.INPUT_METHOD_SERVICE);
                        inputMethodManager4.hideSoftInputFromWindow(edtObservacionServicio.getWindowToken(), 0);
                    }
                }
                else{
        ...
        try {
            ...
            if (con == null) {
                ...
                String query = "select   Descripcion  from CatServicios where tipo=1 order by Descripcion";
                stmt = con.prepareStatement(query);
                rs = stmt.executeQuery();
                ArrayList<String> data = new ArrayList<String>();
                data.add(0,"Seleccionar Mantenimiento..");
                while (rs.next()) {
...
/*
                String query2 = "select distinct Apodo as Nombre from CatPersonal P inner join CatServPersonal "+
                        "SP on SP.CodPersonal=P.CodPersonal inner join CatServicios S on S.Codigo=SP.CodServicio where P.Tipo='M' and S.Tipo=1 order by P.Apodo ";
                stmt = con.prepareStatement(query2);
                rs = stmt.executeQuery();
                ArrayList<String> data2 = new ArrayList<String>();
                data2.add(0,"Seleccionar Mecanico..");
                            ...
                            if (rs.next()) {
                                try{
                                    ...
                                    if (Tamaño > 0) {
                                        ...
                                        AlertDialog.Builder dialog = new AlertDialog.Builder(UnidadServicio.this);
                    ...
                    try {
                        ...
                        if (edtUnidad.getText().toString().equals("")) {
                            Toast.makeText(getApplicationContext(),
                                    ...
                                    Toast.LENGTH_LONG).show();
                            edtUnidad.requestFocus();
                            //}else  if (edtKilometros.getText().toString().equals("")) {
                            //  Toast.makeText(getApplicationContext(),
                            //        "Debe espesificar los kilometros",
                            //      Toast.LENGTH_LONG).show();
                            //edtKilometros.requestFocus();
                            ...
                            //String Chofer = TvChofer.getText().toString();
                            ...
                            /*String query = "declare @CodChofer int, @CodProveedor int,@CodServicio int,@CodMecanico int,@CodOrigen int,@Unidad int;  " +
                                    ...
                                    "if not exists(select * from MqAsignacionServicios where CodMaquinaria=@Unidad and CodSevicio=@CodServicio and Convert(date,FechaSalida)='1999-01-01') " +
                                        "begin " +
                                                "insert into MqAsignacionServicios values (@Unidad,@CodChofer,@CodProveedor,@CodServicio,@CodMecanico,0,'"+ Km +"',0,1,GETDATE(),  " +
                            ...
                            CodServicio=0;
                            edtUnidad.setText("");
                            spServicios.setSelection(((ArrayAdapter)spServicios.getAdapter()).getPosition("Seleccionar Mantenimiento.."));
                            edtObservacionServicio.setText("");
                            spOrigen.setSelection(((ArrayAdapter)spOrigen.getAdapter()).getPosition("Seleccionar Origen.."));
                            spMecanicos.setSelection(((ArrayAdapter)spMecanicos.getAdapter()).getPosition("Seleccionar Mecanico.."));
                            spProveedor.setSelection(((ArrayAdapter)spProveedor.getAdapter()).getPosition("Seleccionar Proveedor.."));
